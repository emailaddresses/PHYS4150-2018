function EVL = eig_householder(matA, max_iter)
...
if N1==2, return; end
for ind1 = 3:N1
    % max_eig = abs(M(ind1)) + abs(M(ind1-1)) + abs(U(ind1-1)) + abs(U(ind1-2)); %no reference for the formula, strange index in lecture note
    max_eig = EVL(end) - EVL(1) + EVL(end);
    min_eig = EVL(1) - (EVL(end) - EVL(1));
